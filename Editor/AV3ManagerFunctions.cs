...
namespace VRLabs.AV3Manager
{
	...
	public static class AV3ManagerFunctions
	{
		public static void InitializeAnimatorsReferences()
		{
			...
			_emptyClip = AssetDatabase.LoadAssetAtPath<AnimationClip>(assetsPath + "/EmptyClip.anim");
		}

		private static Dictionary<AnimLayerType, string> _defaultControllersPath;

		public static Dictionary<AnimLayerType, string> DefaultControllersPath
		{
			get
			{
				if (_defaultControllersPath == null)
					InitializeAnimatorsReferences();

				return _defaultControllersPath;
			}
		}
		...
		private const string DEFAULT_AUTOGENERATED_DIRECTORY = "Assets/VRLabs/GeneratedAssets/";
		...
		public static void AddParameter(VRCAvatarDescriptor descriptor, VRCExpressionParameters.Parameter parameter,
			string directory, bool overwrite = true)
		{
			if (descriptor == null)
			{
			...
			{
				Debug.Log("Directory was not specified, storing new parameters asset in " +
				          DEFAULT_AUTOGENERATED_DIRECTORY);
				directory = DEFAULT_AUTOGENERATED_DIRECTORY;
			}
			...
			{
				if ((descriptor.expressionParameters.CalcTotalCost() +
				     VRCExpressionParameters.TypeCost(parameter.valueType)) >
				    VRCExpressionParameters.MAX_PARAMETER_COST)
				{
					Debug.LogError("Couldn't add parameter '" + parameter.name +
					               "', not enough memory free in the avatar's parameter asset!");
					return;
				}
				...
				if (AssetDatabase.GetAssetPath(descriptor.expressionParameters) !=
				    path) // if we have not made a copy yet
				{
				...
				parameters =
					AssetDatabase.LoadAssetAtPath(path, typeof(VRCExpressionParameters)) as VRCExpressionParameters;
			}
		...
		/// <param name="directory">The unique directory to store the new parameter asset, ex. "Assets/MyCoolScript/GeneratedAssets/725638/".</param>
		/// <param name="overwrite">Optionally, choose to not overwrite an asset of the same name in directory. See class for more info.</param>
		public static void AddParameters(VRCAvatarDescriptor descriptor,
			IEnumerable<VRCExpressionParameters.Parameter> parameters, string directory, bool overwrite = true)
		{
			if (descriptor == null)
			{
			...
			var parametersArray = parameters as VRCExpressionParameters.Parameter[] ?? parameters.ToArray();
			if (descriptor.expressionParameters == null)
			{
			...
			{
				if (descriptor.expressionParameters.CalcTotalCost() +
				    parametersArray.GetCost(descriptor.expressionParameters.parameters) >
				    VRCExpressionParameters.MAX_PARAMETER_COST)
				{
			...
			foreach (var prm in parametersArray)
			{
		...
		public static int GetCost(this IEnumerable<VRCExpressionParameters.Parameter> parameters,
			IEnumerable<VRCExpressionParameters.Parameter> blackList = null)
		{
			return blackList == null
				? parameters.Where(parameter => parameter.networkSynced)
					.Sum(parameter => VRCExpressionParameters.TypeCost(parameter.valueType))
				: parameters.Where(x => !blackList.Any(y => y.name.Equals(x.name)) && x.networkSynced)
					.Sum(parameter => VRCExpressionParameters.TypeCost(parameter.valueType));
		}
		...
		/// <param name="overwrite">Optionally, choose to not overwrite an asset of the same name in directory. See class for more info.</param>
		public static void AddSubMenu(VRCAvatarDescriptor descriptor, VRCExpressionsMenu menuToAdd, string controlName,
			string directory, VRCExpressionsMenu.Control.Parameter controlParameter = null, Texture2D icon = null,
			bool overwrite = true)
		{
			if (descriptor == null)
			{
			...
			{
				Debug.Log("Directory was not specified, storing new topmost menu in " +
				          DEFAULT_AUTOGENERATED_DIRECTORY);
				directory = DEFAULT_AUTOGENERATED_DIRECTORY;
			}
			...
			else
			{
				...
				{
					Debug.LogWarning(
						"Couldn't add menu. Please have an available slot in your avatar's topmost Expression Menu.");
					return;
				}
			...
			{
				if (controlList[i].name.Equals(controlName) &&
				    controlList[i].type.Equals(VRCExpressionsMenu.Control.ControlType.SubMenu))
				{
			...
			VRCExpressionsMenu.Control control = new VRCExpressionsMenu.Control
			{
				...
				parameter = controlParameter, icon = icon
			};
		...
		/// <param name="overwrite">Optionally, choose to not overwrite an asset of the same name in directory. See class for more info.</param>
		[Obsolete(
			"This method has been replaced by the version using the VRC \"AnimLayerType\" and will be removed in future versions of the manager")]
		public static void MergeToLayer(VRCAvatarDescriptor descriptor, AnimatorController controllerToAdd,
			PlayableLayer playable, string directory, bool overwrite = true)
		{
			AnimLayerType layer;
			switch (playable)
			{
				...
				case PlayableLayer.Additive:
					...
					break;
				case PlayableLayer.Gesture:
					layer = AnimLayerType.Gesture;
					break;
				case PlayableLayer.Action:
					layer = AnimLayerType.Action;
					break;
				case PlayableLayer.FX:
					layer = AnimLayerType.FX;
					break;
				default:
					layer = AnimLayerType.FX;
					break;
			...
			MergeToLayer(descriptor, controllerToAdd, layer, directory, overwrite);
		}
		...
		/// <param name="descriptor">The avatar descriptor that merging is being done on.</param>
		/// <param name="controllerToAdd">The controller to merge to the playable layer.</param>
		/// <param name="layer">The playable layer to merge to.</param>
		/// <param name="directory">The unique directory to store the new merged controller, ex. "Assets/MyCoolScript/GeneratedAssets/725638/".</param>
		/// <param name="overwrite">Optionally, choose to not overwrite an asset of the same name in directory. See class for more info.</param>
		public static void MergeToLayer(VRCAvatarDescriptor descriptor, AnimatorController controllerToAdd,
			AnimLayerType layer, string directory, bool overwrite = true)
		{
			if (descriptor == null)
			{
			...
			if (layer != AnimLayerType.FX) // fx layer has no default layer
			{
				if ((AssetDatabase.LoadAssetAtPath(_defaultControllersPath[layer], typeof(AnimatorController)) as
					    AnimatorController) == null)
				{
					Debug.LogError("Couldn't find VRChat's default animator controller at path '" +
					               _defaultControllersPath[layer] + "'! Merging was not performed.");
					return;
				}
			}
			else if (string.IsNullOrEmpty(directory))
			{
				Debug.Log("Directory was not specified, storing new controller in " + DEFAULT_AUTOGENERATED_DIRECTORY);
				directory = DEFAULT_AUTOGENERATED_DIRECTORY;
			}

			CustomAnimLayer[] animatorLayerArray = Array.Empty<CustomAnimLayer>();
			...
			switch (layer)
			{
				...
				case AnimLayerType.IKPose:
					animatorLayerArray = descriptor.specialAnimationLayers;
					break;
			}

			if (animatorLayerArray != null)
			{
				int animatorLayerIndex = -1;
				for (int i = 0; i < animatorLayerArray.Length; i++)
				{
					if (animatorLayerArray[i].type == layer)
					{
						animatorLayerIndex = i;
						break;
					}
				}

				bool alreadyReplaced = false;
				string path = directory + Enum.GetName(typeof(AnimLayerType), layer) + ".controller";
				path = (overwrite) ? path : AssetDatabase.GenerateUniqueAssetPath(path);

				if (animatorLayerArray[animatorLayerIndex].isDefault ||
				    animatorLayerArray[animatorLayerIndex].animatorController == null)
				{
					descriptor.customizeAnimationLayers = true;
					animatorLayerArray[animatorLayerIndex].isDefault = false;

					AssetDatabase.CopyAsset(_defaultControllersPath[layer], path);
					var controllerFromNew =
						AssetDatabase.LoadAssetAtPath(path, typeof(AnimatorController)) as AnimatorController;
					animatorLayerArray[animatorLayerIndex].animatorController = controllerFromNew;
					alreadyReplaced = true;
				}

				if (!alreadyReplaced) // if we have not made a copy yet
				{
					AssetDatabase.CopyAsset(
						AssetDatabase.GetAssetPath(animatorLayerArray[animatorLayerIndex].animatorController), path);
				}
				...
				animatorLayerArray[animatorLayerIndex].animatorController = mergedController;
				AssetDatabase.SaveAssets();
				AssetDatabase.Refresh();
			}
		}
		...
		public static List<WDState> AnalyzeWDState(this VRCAvatarDescriptor descriptor)
		{
			...
			foreach (var layer in descriptor.baseAnimationLayers)
			{
				...
				foreach (var animationLayer in controller.layers)
					AnalyzeWdStateMachine(animationLayer.stateMachine, states, layer.type.ToString(), controller,
						animationLayer);
			}
		...
		public static bool HasMixedWriteDefaults(this VRCAvatarDescriptor descriptor)
		{
			...
			foreach (var layer in descriptor.baseAnimationLayers)
			{
				...
				foreach (var animationLayer in controller.layers)
				{
					(checkedFirst, isOn, isMixed) =
						GetWdInStateMachine(animationLayer.stateMachine, checkedFirst, isOn);
					if (isMixed)
						return true;
				}
			}
		...
		public static bool HaveWDDefaults(this IEnumerable<WDState> states, bool isOn)
		{
			return states.Any(state =>
				state.HasDefault && ((isOn && state.IsDefaultOn) || (!isOn && !state.IsDefaultOn)));
		}
		...
		public static void SetWriteDefaults(AnimatorController controller, bool writeDefaults = true,
			bool force = false, bool ignoreDbts = true)
		{
			if (controller == null)
			{
		...
		public static AnimatorController SwapAnimations(this AnimatorController controller,
			IEnumerable<ClipSwapItem> items, bool saveToNew = false)
		{
			...
			if (saveToNew)
			{
				...
				string uniquePath =
					AssetDatabase.GenerateUniqueAssetPath(AnimatorCloner.STANDARD_NEW_ANIMATOR_FOLDER +
		...
		private static void AnalyzeWdStateMachine(AnimatorStateMachine stateMachine, List<WDState> states,
			string layerName, AnimatorController controller, AnimatorControllerLayer layer)
		{
			foreach (ChildAnimatorState t in stateMachine.states)
			{
				states.Add(new WDState
				{
					...
					HasDefault = t.state.name.ToLower().Contains("(WD On)".ToLower()) ||
					             t.state.name.ToLower().Contains("(WD Off)".ToLower()),
		...
		private static (bool, bool, bool) GetWdInStateMachine(AnimatorStateMachine stateMachine, bool checkedFirst,
			bool isOn)
		{
			foreach (ChildAnimatorState t in stateMachine.states)
			{
				...
				if (t.state.name.ToLower().Contains("(WD On)".ToLower()) ||
				    t.state.name.ToLower().Contains("(WD Off)".ToLower()))
				{
		...
		private static IEnumerable<ClipSwapItem> GetClipsToSwapRecursive(AnimatorStateMachine stateMachine,
			string currentPath)
		{
		...
		public static void SwapClip(AnimatorController controller, ClipSwapItem item)
		{
			...
			{
				stateMachine = stateMachine.stateMachines.Select(t => t.stateMachine)
					.FirstOrDefault(x => x.name.Equals(pathPieces[i]));
