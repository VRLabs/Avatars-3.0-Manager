...
namespace VRLabs.AV3Manager
{
	public static class AnimatorCloner
	{
		...
		public static AnimatorController MergeControllers(AnimatorController mainController,
			AnimatorController controllerToMerge, Dictionary<string, string> paramNameSwap = null,
			bool saveToNew = false)
		{
		...
		private static string GetNewParameterNameIfSwapped(string parameterName) =>
			!string.IsNullOrWhiteSpace(parameterName) && _parametersNewName.ContainsKey(parameterName)
				? _parametersNewName[parameterName]
				: parameterName;
		...
		private static BlendTree CloneBlendTree(BlendTree parentTree, BlendTree oldTree)
		{
			...
			using (var oldSo = new SerializedObject(oldTree))
			{
				...
				{
					pastedSo.FindProperty("m_NormalizedBlendValues").boolValue =
						oldSo.FindProperty("m_NormalizedBlendValues").boolValue;
